...
class MyblogKafkaResourceIT {
    ...
    void setup() {
        ...
        MyblogKafkaResource kafkaResource = new MyblogKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/myblog-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/myblog-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
